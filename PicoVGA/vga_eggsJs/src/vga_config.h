...
#define SEGMAX		2	// max. number of video segment per video strip (size of 1 sSegm = 28 bytes)
#define STRIPMAX	2	// max. number of video strips (size of 1 sStrip = sSegm size*SEGMAX+4 = 228 bytes)
				// size of sScreen = sStrip size*STRIPMAX+4 = 1828 bytes

#define MAXX		512	// max. resolution in X direction (must be power of 4)
#define MAXY		400	// max. resolution in Y direction

#define MAXLINE		600	// max. number of scanlines (including sync and dark lines)
