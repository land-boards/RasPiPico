...
void DispHelp()
{
	...
	printf("Upper left ... discard\n", KEY_DISCARD);
	printf("Fire ... help move\n", KEY_HELP);
	printf("Upper right ... quit\n", KEY_QUIT);
}

// get character from JoyPad and map to keyboard (0 = no key)
// 'J' LEFT=left, 'L' RIGHT=right, SPACE UP=select, 'D' DOWN=discard, 'H' help, 'Q' Fire=quit
char GetChar()
{
	char rVal = 0;
	if (!gpio_get(PB_LT))
